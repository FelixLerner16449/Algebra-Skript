...
\section{Character tables}
    ...
    \end{exercise}
    \begin{proof}
        Since every irreducible representation is contained in the regular representation with the multiplicity of its degree (this is shown right below this exercise), we write $\chi_V = \chi_{\triv} + \chi_{\sgn} + 2\chi_{\mathrm{std}}$, where $\chi_{\mathrm{std}}$ is the representation from exercise \ref{exe:10}. We have
        \begin{equation*}
            \chi_{V^{\tens n}}(g) = \chi_{\mathrm{reg}}^n(g) = \#G^n \delta_{g,e}
        \end{equation*}
        Hence we get a decomposition
        \begin{equation*}
            \chi_{V^{\tens n}} = \#G^{n-1} \chi_{\mathrm{reg}}(g) = \#G^{n-1}(\chi_\triv + \chi_\sgn + 2 \chi_\mathrm{std}) = 6^{n-1}(\chi_\triv + \chi_\sgn + 2 \chi_\mathrm{std})
        \end{equation*}
        since the irreducible character form a basis of $\CF(G)$, this is the decomposition of $V^{\tens n}$. Note that this technique works for any group.
    \end{proof}
