...
\section{Semisimple modules and algebras}
    ...
    \subsection{Hom-spaces}
        ...
        The big direct sum can be organized in a matrix form. Let $M$ be an $n \times n$ matrix whose $(i,j)$-th entry is from $\Hom_A(M_j,M_i)$ ({\color{green}I believe this is the correct way, in the notes it says $\Hom_A(M_i,M_j)$ but I don't think this is right. Just write out the case $k=2$}). Moreover, we can multiply $M$ with another Matrix $M'$ of this form where the usual multiplication of scalars is replaced by the composition of maps. This gives a $\C$-algebra structure on the right-hand side.
