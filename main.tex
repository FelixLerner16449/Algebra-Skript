...
\section{Projector, Maschke and Schur}
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            \item Assume that $f:V\cong_G W$. For any $t\in\C$, $f\inv\circ\phi-t\cdot\id_V\in\Hom_G(V,V)$ and hence $\ker(f\inv\circ\phi-t\id_V)$ is a subrepresentation of $V$. Let $\lambda$ be an eigenvalue of $f\inv\circ\phi:V\to V$. Then $\ker(f\inv\circ\phi-\lambda\id_V=V$ because $V$ is irreducible. That is to say, $\phi=\lambda f$.
...
\section{Left regular representation}
    ...
    \end{proof}
    (same trick used in the proof of theorem \ref{thm:2.19} in the claim)
...
\chapter{Semi-simple algebras}
    In the last chapter we have seen that studying the representation theory of groups is equivalent to the study of modules over the group algebra $\C[G]$, see Exercise \ref{exc:12}.
...
\section{Modules over algebras}
    \subsection{Modules}
        In Computeralgebra \cite{Cobra} we have already studied modules over (commutative) rings. In the following we particularly study the non-commutative setting.
        ...
        Returning to the lecture notes, we define modules over $K$-algebras just as in the commutative case, we just have to make a difference between left and right modules.
        ...
        \end{remark}
        Sometime we will simply write $\tensor[_A]{M}{}$ or $M_A$ to emphasize that it is a left or right $A$-module. When we just say $A$-module we mean left module by default.
        ...
        \begin{definition}{}{3.5}
            \begin{enumerate}
                ...
                \item
                    A subspace $I \subseteq A$ is called an ideal (or a two-sided ideal), if it is both a left and a right ideal in $A$.
            \end{enumerate}
        \end{definition}
