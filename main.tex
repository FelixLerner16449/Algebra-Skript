...
\section{Galois extensions}
    ...
    \subsection{Algebraic extensions}
        ...
        In general the extension $L/K$ is finite if and only if there exist $\alpha_1,\dots,\alpha_k \in L$ such that $L = K(\alpha_1,\dots,\alpha_k)$.
        ...
        \begin{proof}
            ...
            For the sake of contradiction let us assume that $\sqrt{p_s} \in \Q(\sqrt{p_1},\dots,\sqrt{p_{s-1}})$. Because of \eqref{*} there exist $q,a_1,\dots,a_{s-1} \in \Q$ where we assume without loss of generality that $a_{s-1} \neq 0$:
            \begin{align*}
    ...
    \subsection{Galois extensions}
        ...
        So to get a Galois extension, we need the minimal polynomial to split into linear factors.
        ...
        The definition of the Galois extension is not easy to use when determining whether a given extension is Galois or not, because computing $\Gal{L/K}$ is usually a hard task. The following proposition gives a convenient way to determine whether an extension is Galois.
        ...
        We finish this section with the following examples. Computations of Galois groups will be discussed later.
...
\section{Galois correspondence}
        In this section, we will state and prove the fundamental theorem of Galois theory. Roughly speaking it gives a correspondence (called Galois correspondence) between the intermediate fields in a Galois extension and the subgroups of the Galois group. This correspondence translates problems in field theory to group theory, which is the key idea of Galois.
    ...
    \subsection{Fundamental theorem}
        ...
        \begin{notation*}
            Let $G$ be a group. We denote the set of its subgroups as $\Gamma(G) := \{U \ | \ U \leq G\}$. The set $\Gamma(G)$ has a partial order:
            \begin{align*}
        ...
        So (2) gives us (in comparison to \ref{cor:1.3}) that the lower extension is also Galois. It is a characterization of when this is the case.
...
\section{Examples}
    ...
    \subsection{An easy example and Frobenius}
        ...
        \begin{example}{}{1.19}
            ...
            We first compute $\eckig{L:\Q}$. For this we consider the intermediate field $\Q(\zeta)$ and $\Q(\sqrt[3]{2})$. The minimal polynomial of $\zeta$ over $\Q$ is $x^2 + x + 1 \in \Q[x]$ and the minimal polynomial of $\sqrt[3]{2}$ over $\Q$ is $x^3 - 2 \in \Q[x]$. We set $a:= [L:\Q(\zeta)]$ and $b:=[L:\Q(\sqrt[3]{2})]$. Then by the degree formula $2a = 3b$ and hence $2 | b$. But $\zeta \not \in \Q(\sqrt[3]{2}) \subseteq \R$ so $[L:\Q(\sqrt[3]{2})] \geq 2$ hence $b = 2$, which means $[L:\Q] = 6$.
            ...
            The Galois group $\Gal{L/K}$ is the symmetric group of the triangle, also known as $D_3$ (or $D_6$ if you are a group theorist).
...
\section{Proof of Galois' theorem}
   ...
   {\color{blue}The generator $\ell$ in the proof of the lemma can be written in an explicit form, see Corollary \ref{cor:1.35}. This explicit form, called Lagrange resolvent, will be very helpful in finding solutions to cubic equations by radicals.}
    ...
    We start from Lagrange resolvent. We keep notations as in Lemma \ref{lem:1.33} with $G:=\Gal{L/K}$.
    
    Let $L^\times:=L\setminus\{0\}$ be the multiplicative group of the field $L$. A \textbf{character} of $G$ (with value in $L^\times$) is a group homomorphism $\chi:G\to L^\times$. We denote $\mathrm{Ch}(G,L^\times)$ the set of such characters. Note that these characters are different to those we will see in group representation theory.
    ...
    \begin{corollary}{}{1.35}
        ...
        This element $\ell(a)$ in the Lemma is called a \textbf{Lagrange resolvent}.
    \end{corollary}
...
\section{Linear representations}
    ...
    \subsection{Definition and examples}
        ...
        \begin{example}{}{2.4}
            Let $G \circlearrowleft X$, i.e. $X$ is a finite $G$-set. By definition, there exists a homomorphism of groups
            \begin{equation*}
            \begin{split}
            ...
            We consider the vector space $\C(X)$ which is the free vector space of $X$:
            \begin{equation*}
...
\separline{Week 5}
        ...
        Note that for a representation $(V,\rho_V)$, there are always the two subrepresentations $(\{0\},\rho_V|_{\{0\}})$ and $(V,\rho_V)$.
...
\subsection{Central functions}
    ...
    \end{definition*}
    Since $\C[G]$ is a monoid with multiplication, $Z(\C[G]) = \{x \in \C[G] \ | \ \forall y \in \C[G]: xy = yx \}$.
    ...
    \begin{proof}
        ...
        \end{equation*}
        It suffices to show that $\sum_{g \in G} \alpha(g) h g h^{-1} = \sum_{g \in G} \alpha(g)g$.
        ...
        Thus we obtained a $\C$-algebra homomorphism $\CF(G) \to Z(\C[G])$ which is injective (as the restriction of an isomorphism). We show that it is also surjective:

        Take $x = \sum \lambda_g g \in Z(\C[G])$ and $h \in G$, since $hxh^{-1} = x$ it follows that
        \begin{equation*}
            h\round{\sum_{g\in G} \lambda_gg}h\inv= \sum_{g \in G} \lambda_g h g h^ {-1} = \sum_{g \in G} \lambda_g g
        \end{equation*}
    ...
    \begin{corollary}{}{2.16}
        The $\C$-vector space $Z(\C[G])$ has a basis $\curly{\sum\limits_{g \in C} g \ \Bigg| \ C \in \cC(G)}$. In particular $\dim_\C(Z(\C[G])) = \# \cC(G)$.
    \end{corollary}
    \begin{proof}
        Combining Corollary \ref{cor:2.14} and Lemma \ref{lem:2.15}, $\curly{\gamma_C^\circ \ \Bigg| \ C \in \cC(G)}$ forms a basis of $\Z(\C[G])$. It suffices to notice that $\gamma_C^\circ = \sum\limits_{g \in G} \gamma_C(g)g = \sum\limits_{g \in G} g$
    \end{proof}
...
\separline{week 12}
    ...
    \textbf{Special case:}
        ...
        We explain it in a numerical way using characters, where the name ``reciprocity'' is clear. We let $\spitz{-,-}_G$ and $\spitz{-,-}_H$ be the hermitian forms on $G$ and $H$ respectively.
        ...
        If we take $G=S_n$, $H = S_{n-1} \subseteq S_n$, the embedding of $S_{n-1}$ in $S_n$ from the inclusion $[n-1] \hookrightarrow [n]$, then we get
        \begin{theorem}{}{4.7}
...
\subsection{Projective resolutions}
    ...
    We have studied a single module, a homomorphism (two modules). Next, we move to three modules.
