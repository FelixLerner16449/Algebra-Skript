...
\setcounter{section}{-1}
\section{Non commutative tensor product}
    For the sake of completeness I (Felix) added this chapter. This is not actually part of the notes but I think this is actually nessecary for understanding the following sections in particular, why do we take $R$-structure on only one component of the tensor product.

    Let us set the stage. Take a ring $R$ (unital and associative ofcourse). Now if $R$ is commutative, then we define the tensor product of two $R$-modules $M,N$ exactly as before:
    ...
    \end{equation*}
    where $\star$ denotes the point, where we assumed the $R$-module structure to be associative. Thus we only get an abelian group $M \tens N$.
...
\section{Simple modules and algebras}
    We first remove "semi" from the title of this chapter and study finite-dimensional simple algebras over $\C$. In the rest of this chapter, we fix $\C$ as the base field, although any algebraically closed field of characterstic $0$ would make do.
...
\section{Semisimple modules and algebras}
    ...
    \begin{remark*}{a comment on semisimplicity - F}
        ...
        This also holds for infinite dimensional modules. The proof uses Zorns Lemma.
    \end{remark*}
    ...
    \end{exercise}
    \begin{proof}
        We proof it for any $A$-module $M$, for the finite dimensional case we dont need Zorns Lemma, but we just argue by dimension. If you need more details on the Zorns Lemma proofs just ask me (Felix).
    ...
    \begin{proof}
        The direction $"\Rightarrow"$ follows from the defintion.
    ...
    \begin{theorem}{Artin-Wedderburn}{ArWb}
        Let $A$ be a finite dimensional $\C$-algebra. The following statments are equivalent:
        \begin{enumerate}
    ...
    \subsection{Hom-spaces}
        ...
        According to Example \ref{exa:3.7}, $\End_A\round{\tensor[_A]{A}{}} \cong A^\op$. So we investigate $\End_A\round{\bigoplus\limits_{i=1}^k S_i^{\oplus n_i}}$. For this we consider the following general setup. Let $M_1,\dots,M_k$ be $A$-modules. The we have an isomorphism of $\C$-vectorspaces
        \begin{equation*}
        ...
        How about composition of endomorphisms?

        The big direct sum can be organized in a matrix form. Let $M$ be an $n \times n$ matrix whose $(i,j)$-th entry is from $\Hom_A(M_j,M_i)$ ({\color{green}I believe this is the correct way, in the notes it says $\Hom_A(M_i,M_j)$ but I dont think this is right. Just write out the case $k=2$}). Moreover, we can multiply $M$ with another Matrix $M'$ of this form where the usual multiplication of scalars is replaced composition of maps. This givs a $\C$-algebra structure on the right hand side.
        ...
        Applying this general setup to the aboe special case and noticing that by Schurs Lemma (\ref{lem:schurMOD}), $\Hom_A(S_i,S_j) = \{0\}$ for $i \neq j$. We have in fact an isomorphism of $\C$-algebras
        \begin{equation*}
            \End_A\round{S_i^{\oplus n_i}} \cong \bigoplus_{1 \leq p,q \leq n_i} \End_A(S_i) \cong \bigoplus_{1 \leq p,q \leq n_i} \C
        \end{equation*}

        Writing this isomorphismin a matrix form as above, we obtain an isomorphism of $\C$-algebras $\End_A\round{S_i^{\oplus n_i}} \cong M_{n_i}(\C)$. It follows then, that there exists an isomorphism of $\C$-algebras:
        \begin{equation*}
        \begin{split}
    ...
    \subsection{Isotypic decomposition}
        We prove $(2)\Rightarrow(1)$ by showing that if $A$ is a product of matrices, then every $A$-module is semisimple. We start from classifying simple $A$-modules. Let $\Irr(A)$ denote the set of \underline{simple} $A$-modules up to isomorphism.
...
\section{Proof of the Schur-Weyl Duality}
    ...
    \begin{proof}[Proof of Theorem \ref{thm:SW}]
        It follows from definition that $B\subseteq C(A)$ and $A\subseteq C(B)$. If we can prove that $B=C(A)$, it follows from the double commutant theorem that $B$ is semisimple and $A=C(B)$.
