...
\section{Integrality and divisibility}
    ...
    \begin{corollary}{}{2.36}
        \begin{enumerate}
            ...
            \item For any $V\in\rep(G), \chi_V:Gto\C$ has image in $\bar\Z$
        \end{enumerate}
    \end{corollary}
    ...
    \begin{corollary}{}{2.37}
        For any $C\in\cC(G)$, the element $\gamma_C^o\in Z(\C[G])$ is integral over $\Z$.
    \end{corollary}

    \begin{proof}
        Let $\cC(G)=\curly{C_1,\dots,C_N}$. we consider the $\Z$-submodule $R:=\Z\gamma_{C_1}^o\oplus\dots\oplus\Z\gamma_{C_l}^o\subseteq Z(\C[G])$. From the multiplication formula $\gamma_{C_i}^o\cdot\gamma_{C_j}^o=\sum_{k=1}^lm_{ij}^k\gamma^o_{c_k}$ where $M_{ij}^k:=\#\curly{(g,h)\in C_i\times C_j| gh\in \C_k}$ (verify it!).

        It follows that $R$ is a finitely generated $\Z$-submodule of $Z(\C[g])$ which contains $\Z[\gamma_{C_i}^o]$ for any $i\in [l]$.
    \end{proof}
    ...
    \begin{proof}
        Since $V$ is irreducible and $\gamma_C^o\in Z(\C[G])$, it follows from Schurs lemma (\ref{cor:2.27} to be precise) that $\tilde\rho_V(\gamma^o_C)=\sum_{h\in C}\rho_V(h)$ \textit{is} a scalar $\lambda\cdot \id_V$ for $\lambda\in\C$. To determine $\lambda$, we take the trace and obtain:
        $$\lambda\dim V=\sum_{h\in C}\chi_V(h)=\#C\chi_V(g)$$ for any $g\in C$. It remains to show: $\lambda \in\bar \Z$.
        According to \ref{cor:2.37}, $\gamma_C^o$ fulfills an equation
        $$(\gamma_c^o)^m+a_{m-1}(\gamma_c^o)^{m-1}+\dots+ a_1\gamma_C^o+a_0=0$$
...
\section{Bursides' theorem}
    ...
    \begin{proof} Theorem~\ref{thm:2.44}
        ...
        By Exercise~\ref{exc:8}(4), $G$ is solvable.
        ...
        Hence $[G:C_G(g)]|[G:P] = q^b$. Therefore $[C:C_G(g)]$ is a power of $q$, say $q^r$. Notice that $[G:C_G(g)]$ is the cardinality of the conjugacy class containing $g$ (by the orbit-stabilizer theorem).
...
\chapter{Semi-simple algebras}
    In the last chapter we have seen that studying the representation theory of groups is equivalent to studying modules over the group algebra $\C[G]$, see Exercise \ref{exc:12}.
...
\subsection{Simple modules}
    ...
    \begin{example}{}{3.14}%pi :)
        ...
        Let $E_{i,j} \in M_n(\C)$ be the elementary matrix, whose only non-zero entry is a $1$ at entry $i,j$. To prove that $M_n(\C)$ is simple, we choose a non-zero matrix $M \in M_n(\C)$ with $M = (m_{i,j})_{i,j}$ and show that $AMA = M_n(\C)$ (here $AMA = \curly{XMY \ | \ X,Y \in M_n(\C)}$), is the smallest two-sided ideal containing $M$.
